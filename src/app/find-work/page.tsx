...
function FindWorkContent() {
  ...
  const filterProjects = useCallback(() => {
    ...
    if (filters.role) {
      filtered = filtered.filter(project =>
        project.rolesNeeded.includes(filters.role)
      )
    }
    ...
    if (sortBy === 'start-date') {
      filtered.sort((a, b) => new Date(a.shootStartDate).getTime() - new Date(b.shootStartDate).getTime())
    } else if (sortBy === 'applications') {
      filtered.sort((a, b) => a._count.applications - b._count.applications)
    } else {
      // newest (default)
      filtered.sort((a, b) => new Date(b.shootStartDate).getTime() - new Date(a.shootStartDate).getTime())
    }
  ...
  // Initialize filters from URL query params (role, projectType, location, dateRange)
  useEffect(() => {
    const role = searchParams.get('role') || ''
    const projectType = searchParams.get('projectType') || ''
    const location = searchParams.get('location') || ''
    const dateRange = searchParams.get('dateRange') || ''
    if (role || projectType || location || dateRange) {
      setFilters(prev => ({
        ...prev,
        role,
        projectType,
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          <div>
            ...
            <p className="text-gray-600 mt-2">
              Browse available projects and apply to gigs that match your skills
            </p>
          </div>
          <Button 
                    ...
                    <DialogHeader className="pb-4 border-b border-gray-200">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600 mt-1">Find projects that match your skills and preferences</p>
                        </div>
                      </div>
                    </DialogHeader>
                    <div className="mt-6">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="flex items-center space-x-2">
                  <p className="text-sm text-gray-600">
                    Showing <span className="font-semibold text-gray-900">{startIndex + 1}-{Math.min(endIndex, filteredProjects.length)}</span> of <span className="font-semibold text-gray-900">{filteredProjects.length}</span> projects
                  </p>
                  {getActiveFilterCount() > 0 && (
